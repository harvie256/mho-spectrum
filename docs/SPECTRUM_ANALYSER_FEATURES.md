...
| Y units: V, dBV, dBmV, dBµV | Absolute voltage units derived from the scope's own preamble scaling | `Units` (Rigol/Keysight/Siglent/R&S). The tap now fills the header's `yincrement`/`yorigin`/`yreference` (`mhotap_set_yscale`, queried once at startup) and the VIEW tab offers **dBV** as a constant offset from dBFS (`analysis.unit_offset_db`). V, dBmV and dBµV are not offered; the SCPI and synthetic sources send no scale, so they stay in dBFS | Partial | S | High |
...
| Feature | What it does | Vendor terminology / notes | Status | Effort | Value |
|---|---|---|---|---|---|
| Preamble-based volts scaling | Convert raw ADC codes to volts using the scope's own `yincrement`/`yorigin`/`yreference` | Implemented for SCPI in `device/rigol_mho.py`. The tap now fills the header fields (`device/tap_stream.py` queries them once at setup, `mhotap_set_yscale` stores them) and `stream_client.Frame` carries them; the GUI uses `yinc` for dBV/dBm. Still partial: read once, so a V/div change mid-session is not picked up, and `ScpiSource` does not populate them | Partial | M | High |
