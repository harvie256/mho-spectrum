...
user, graded against `fftdemo/` as it stands, so we can pick items off a list rather than
...
- **Not gap-free, and not close.** The deep record only exists while the scope is
  ...
  Anritsu's practice of showing POI and minimum-detectable-duration as live status
...
- **The scope's front end sets dynamic range, not the FFT.** 12-bit ADC, and measured on
  ...
  `docs/STREAMING.md`), harmonic products around −67 to −76 dBFS, and a per-bin noise
...
average, and −peak". Adding the other detectors is one combo box over the same code path.
...
hardware mask triggering are properties of the acquisition pipeline, and ours has a sub-1%
duty cycle.
...
| Probability of intercept / real-time BW | Guarantee that any event above a stated duration is captured with full amplitude accuracy | Requires every sample transformed with ≥50% overlap, indefinitely. At <1% duty cycle this is not approximable | N/A (hardware) | – | – |
...
| SFDR | Ratio of the fundamental to the largest spur | baudline's Distortion group. **Would have quantified the fs/8 and fs/4 interleave spurs immediately** (measured −60 dBFS, `docs/STREAMING.md`) | Missing | S | High |
...
| AM/FM demodulation and audio | Listen to the signal at the marker | `Demod` with volume and dwell (Rigol), `Marker Demodulation` (R&S). Needs a DDC and audio output; the <1% duty cycle makes continuous listening impossible | Missing | L | Low |
...
| Feature | What it does | Vendor terminology / notes | Status | Effort | Value |
|---|---|---|---|---|---|
| Preamble-based volts scaling | Convert raw ADC codes to volts using the scope's own `yincrement`/`yorigin`/`yreference` | Already implemented in `capture/rigol_mho.py`; the stream header **reserves the fields but the tap leaves them zero**. This is the gate on every absolute unit | Missing | M | High |
...
| Soft-key menu hierarchy | The FREQ / AMPT / BW / TRACE / MARKER / MEAS structure itself | Worth adopting as the top-level organisation once there are more than a dozen controls — it is the mental model every user of an analyser already has | Missing | M | Med |
...
- Peak table with sort and CSV export
...
- Instrument-style annotation block, with manual-setting indicators
...
- Pause/freeze that keeps the display interactive
...
- Runtime channel selection, and scope sample-rate/memory-depth control from the app
...
- **Probability of intercept, real-time bandwidth, gap-free monitoring** — under 1% duty
  cycle; the numbers would be fiction.
...
- **Full demodulation applications (AM/FM audio, constellation, EVM, symbol tables)** —
  a separate application, and unusable anyway at a sub-1% duty cycle.
